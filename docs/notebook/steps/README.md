...
You can pass data between steps using step exports. Steps can export immutable, JSON-serializable values using `return` or `this.foo = 'bar'`. Step exports are mapped to the `steps` object at `steps.unique_step_name`.
...
## Referencing trigger events in code steps
